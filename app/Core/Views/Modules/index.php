...
<div class="container-fluid">
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body p-0">
                    ...
                    if (isset($modules) && is_array($modules)) {
                        log_message('debug', 'Modules data: ' . print_r($modules, true));
                        foreach ($modules as $m) {
                            if (!isset($m['id']) && $m['id'] !== 0) {
                                log_message('error', 'Module with missing or invalid ID: ' . print_r($m, true));
                            }
                        }
                    } else {
                        log_message('error', 'No modules data or invalid format: ' . gettype($modules));
                    }
                    ?>
    ...
    <?php if (!$isCore): ?>
        ...
        // Ensure moduleId is properly set, even if it's 0
        $moduleId = $moduleId === null ? 0 : (int)$moduleId;
        
        log_message('debug', "Rendering delete button for module ID: {$moduleId}, Name: {$moduleName}");
        ?>
        <button type="button" 
                class="btn btn-action text-danger delete-module" 
                data-module-id="<?= $moduleId ?>" 
                data-module-name="<?= htmlspecialchars($moduleName, ENT_QUOTES, 'UTF-8') ?>"
                data-bs-toggle="tooltip" 
                title="Delete"
                onclick="console.log('Delete clicked:', {id: <?= $moduleId ?>, name: '<?= htmlspecialchars($moduleName, ENT_QUOTES, 'UTF-8') ?>'})">
            <i class="fas fa-trash"></i>
        </button>
        <script>
        // Temporary debug - remove after fixing
        document.addEventListener('DOMContentLoaded', function() {
            const btn = document.querySelector('[data-module-id="<?= (int)$moduleId ?>"]');
            if (btn) {
                console.log('Button found with data:', {
                    id: btn.dataset.moduleId,
                    name: btn.dataset.moduleName,
                    element: btn
                });
            } else {
                console.error('Button not found for module ID:', <?= (int)$moduleId ?>);
            }
        });
        </script>
    <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle delete module button clicks
    document.querySelectorAll('.delete-module').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Debug log
            console.log('Delete button clicked:', { moduleId, moduleName });
            
            // Convert ID to number (allow 0)
            moduleId = moduleId === '0' ? 0 : parseInt(moduleId, 10);
            
            // Validate module ID
            if (isNaN(moduleId)) {
                console.error('Invalid module ID (not a number):', moduleId);
                Swal.fire('Error', 'Invalid module ID', 'error');
                return;
            }
            
            if (!moduleName) {
                console.error('Missing module name');
                Swal.fire('Error', 'Missing module name', 'error');
                return;
            }
            
            confirmDelete(moduleId, moduleName);
        });
    });
...
function confirmDelete(moduleId, moduleName) {
    // Debug log
    console.log('confirmDelete called with:', { moduleId, moduleName });
    
    // Convert to number if it's a string (allow 0)
    moduleId = moduleId === '0' ? 0 : parseInt(moduleId, 10);
    
    // Validate module ID (0 is valid)
    if (isNaN(moduleId) || moduleId === null || moduleId === undefined) {
        console.error('Invalid module ID:', moduleId);
        Swal.fire('Error', 'Invalid module ID', 'error');
        return;
    }
    
    console.log('Deleting module:', { moduleId, moduleName });
    ...
    Swal.fire({
        ...
        preConfirm: async () => {
            try {
                ...
                const url = `<?= site_url('modules/delete/') ?>${moduleId}`;
    ...
    if (result && result.value) {
        Swal.fire({
            ...
            // Remove the module row dynamically instead of reloading
            const row = document.querySelector(`button[data-module-id="${moduleId}"]`)?.closest('tr');
            if (row) {
                row.remove();
            }
        });
