...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-tasks me-2"></i>PLC Applications Management
        </h1>
    </div>

    <!-- Tab Navigation -->
    <ul class="nav nav-tabs" id="plcTabs" role="tablist">
        <li class="nav-item">
    ...
    <!-- Tab Content -->
    <div class="tab-content mt-3" id="plcTabContent">
...
document.addEventListener("DOMContentLoaded", function () {
    function loadApplications(tabId, status) {
        ...
        fetch(`<?= site_url('admin/plc/applications') ?>?status=${status}`)
    ...
    document.querySelectorAll("#plcTabs a").forEach(tab => {
...
$(document).ready(function() {
    ...
    function loadApplications(status) {
        ...
        $.ajax({
            url: '<?= site_url('admin/plc/applications/get') ?>',
            method: 'GET',
            data: { status: status },
            dataType: 'html',
    ...
    const activeTab = document.querySelector('#plcTabs .nav-link.active');
    if (activeTab) {
    ...
    $(document).on('click', '.btn-update-status', function() {
        ...
        $.ajax({
            url: `<?= site_url('admin/plc/applications/update-status') ?>/${applicationId}`,
            method: 'POST',
            data: { 
                status: newStatus,
